...
def in_gene(headers):
    ...
    for header in headers:
        print(10,header.data,type(header.id))
        ...
        if header.data=='if': #如果是if节点
            ...
            # for full multiply
            if headers[0].data=='*':
                a='%s  :  [%s,%s] = [%s,%s]'%(header.id+2,header.nchild[0].num,header.nchild[0].id+'_',\
                                       header.achild[0].num,header.achild[0].id+2)
                cod_for_print.append(a)
                co=incd()
                co.num=header.id+2
                co.type='='
                co.in1=[header.nchild[0].num,header.nchild[0].id+'_']
                co.in2=[header.achild[0].num,header.achild[0].id+2]
                cod.append(co) 
            ...
            cod.append(co)
            if header.data == '*':
                a='%s  :  [%s,%s] %s [%s,%s]'%(header.id+2,header.fchild[0].num,header.fchild[0].id,\
                                        '~',header.tchild[0].num,header.tchild[0].id)
                cod_for_print.append(a)
                co=incd()
                co.num=header.id+2
