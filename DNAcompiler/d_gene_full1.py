...
def mul(inp1,inp2,oup):
    ...
    if poin['and']>1009 or poin['xor']>1023:
        ...
        poin['xor']+=1
        return tmp_table
def mul1(inp1,inp2,oup):
    global poin
    tmp_table=[]
    if poin['and']>1009 or poin['not']>1017:
        print('ERROR: Space Not Enough!!!')
        return -1
    else:
        addr=[['id',poin['and']],poin['and']]
        tmp_table.append([addr,[inp1[1],inp2[1]],poin['and']])
        poin['and']+=1
        
        #second out wire
        addr=[['id',poin['not']],poin['and']+1]
        tmp_table.append([addr,[[['id',poin['and']-4]]],poin['and']+1])
        poin['not']+=1
        
        addr=[['id',poin['and']],oup[1]]
        tmp_table.append([addr,[['id',poin['and']-4],['id',poin['and']-1]],oup[1]])
        poin['and']+=1
        
        addr=[['id',poin['and']],oup[0]]
        tmp_table.append([addr,[['id',poin['and']-2],['id',poin['not']-1]],oup[0]])
        poin['and']+=1
    ...
    for co in nincode:
        ...
        if co.type=='if':#对if语句的拆解
            ...
            if co.type=='+':
                ...
                tmp_table.extend(mul1(inp1,inp2,oup))
##        else :
##            addr=[poin[co[3]],co[0]]
...
#rename overflowed bits
    for ins in instr:
        
        if '__0'in ins:
            ind= instr.index(ins)
            tmp=ins.replace('__0','_2')
            instr[ind]=tmp
            
        elif '__1' in ins:
            ind= instr.index(ins)
            tmp=ins.replace('__1','_3')
            instr[ind]=tmp
    return instr
